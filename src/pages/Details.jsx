...
function Details(props) {
  ...
  const fetchSeasonList = async (platform, seriesid, seasonnumber) => {
    const response = await instance.get(
      apiRequests.getSeasonList(platform, seriesid, seasonnumber)
    );
  ...
  return (
    <div className="py-24 text-white">
      ...
      <div className="flex gap-4 mt-8">
        ...
        <div className="w-2/3">
          <div className="mb-4">
            <h1 className="font-display text-6xl mb-5">
              {data?.title ||
                data?.original_title ||
                data?.name ||
                data?.original_name}
            </h1>
            <h3 className="text-4xl font-alternate text-yellow-500 mb-4">
